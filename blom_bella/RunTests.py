from blom_bella import MainStarter
...
def printError(resultLine, ansLine):
    ...
    if(not CHECK_LEFT_SPACING):
        resultLine = resultLine.lstrip()
        ansLine = ansLine.lstrip()
...
def addInput(redirectedOut, redirectedIn, fileOutName) :
    ...
    #check each line for an input pattern
    lines = []
    while i < len(result):
        line = result[i] + " "
        ...
        for  x in range(1, len(tokens) ):
            ...
            try:
                ...
                while len(next)==0 or next[ 0 ] =="#":
                    next = inFileLines[j].strip()
                    if SHOW_REDIRECTED_INPUT_COMMENTS and len(next) > 0 and next[0] =="#":
                        newLine += ("\n" + ColorText.fg.cyan + ColorText.bold + next + LINE_MARKER
                                    + ColorText.reset+"\n")
                    j += 1
            except:
...
def printErrors(answerFile, outFile):
    ...
    if MAX_ERROR_LINES > errorCnt:
        ...
        #student output has remaining lines
        if j < len(resultLines):
            sys.stderr.write(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Student output has more lines>>>>>>>>>>>>>>>>>>>>>>>\n")
        ...
        # solutions output has remaining lines
        if j < len(ansLines):
            sys.stderr.write(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Solution has more lines>>>>>>>>>>>>>>>>>>>>>>>\n")
...
if __name__ == '__main__':
    ...
    tests = ["../tier1","../tier2","../tier3","../tier4",
             "../tier5","../tier6","../tier7","../tier8a",
             "../tier8b","../tier9","../tier10a","../tier10b","../tier11"]
